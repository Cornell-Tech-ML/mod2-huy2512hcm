...
def backpropagate(variable: Variable, deriv: Any) -> None:
    ...
    derivative = {id(variable): deriv}

    for var in order:
        if var.is_leaf():
            var.accumulate_derivative(derivative[id(var)])
        else:
            for scalar, d in var.chain_rule(derivative[id(var)]):
                if id(scalar) not in derivative:
                    derivative[id(scalar)] = 0
                derivative[id(scalar)] += d
